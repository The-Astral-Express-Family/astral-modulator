...
## A（续）. 第 37 轮（2026-09-16，Lidozs55）：TODO v2 重写——面向一次性完整实施的交付版

用户裁决：将剩余全部工作（Phase 5 + Phase 6 余量）交给一个弱模型做**一次性
完整实现**，因此登记簿从「协作者视角的任务/裁决登记中心」重写为「实现者视角
的实施总纲」——不能有开口裁决、不能依赖轮次记忆、每项工作自含验收标准。

本轮工作：

1. **全部开口项裁决闭口**（详见新 TODO.md §1.2）：M1（memory 无独立 API：
   `memory/` 前缀 + org-memory 保留 workspace + bootstrap 惰性种子）、
   T1（无 DELETE task，cancelled 可复活）、P1（push 双改不做服务端自动合并，
   diff3 选型关闭）、P2（tombstone 删除语义）、P3（不建文档历史表）、
   L1/E1（长度口径=字节；错误码配对规则表）、A1'（message.send 豁免 audit）、
   F1（fuzzy-only 剔除 0 分行）、G1（列表信封统一）、U1（不引入 UI 组件库）、
   X1（SMTP/CLI-register/config-TOML/pg_trgm/平台角色可选项移出范围）。
2. **契约先行落地**：openapi.yaml 8 处增补（documents DELETE、conflicts
   详情端点、/admin/audit、四列表端点分页信封与过滤参数、DocumentConflict
   扩展 + DocumentConflictDetail、Document/ManifestItem 增 deleted、
   AuditEntry 增 request_id）；服务端对应 501 桩（document delete/getConflict、
   admin audit）保持契约门绿；scopes.go 增 platform:audit:read（admin bundle）。
   全量 `go test ./... -count=1` 绿后收口。
3. **TODO.md 重写为实施总纲**（§0 硬约束与验证命令 / §1 决策速查 / §2 实施计划
   S1-S8 / §3 不做清单 / §4 代码内 TODO 处置映射 / §9 契约登记 / 附录 A 稳定
   锚点）。附录 A 保留原 §2 表与原 §11 编号清单（编号不变），§9 编号保留——
   代码与 docs 的既有引用（sse.go/admin/module.go/device.go/registration.md/
   adr-0008/DeviceApproveView 等）全部继续可解析；仅 audit/module.go 的
   「§3.2」引用改指 S4-3。
4. **旧登记簿全文移入本卷 §D**（只移不改；git show HEAD:TODO.md 逐字节）。
5. **docs 校准**：architecture（§6.5/§10.1/§15/§16/§27）、protocol（§3 分页
   与长度口径）、sync-semantics（§8/§9/§13/§14 裁决落档 + 状态转 Accepted）、
   registration（P3/P4 状态）、roadmap（进度指针）、deployment（memory 路由
   说明与 server.toml 展望）、README/MANIFEST（TODO.md 角色描述）。

维护规则变化：实现完成后回到人工维护——勾选总纲 §2、轮次详录照旧写本卷 §A
卷末；契约变更照旧登记总纲 §9（历史 40 行在本卷 §D）。

## D. 登记簿原卷（round 36 版 TODO.md 全文；2026-09-16 随第 37 轮归档，只移不改）

> 以下是重写前的 TODO.md 原文（提交 bdabb31 时刻），仅作历史与裁决理由存档；
> 现行文档以根目录 TODO.md（实施总纲）为准。原文中的「§3.2」等节号在本卷
> 语境下指本文件内的小节，引用请以总纲附录 A 的稳定编号为准。
