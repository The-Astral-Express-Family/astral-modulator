...
export function listWorkspaceChildren(
  ...
  return apiFetch(
    `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/children?${buildQuery(params)}`,
  )
}

// task 容器的子任务集合 = 直接子层。
export function listTaskChildren(taskId: string, params: TaskFilterParams = {}): Promise<Page<Task>> {
  return apiFetch(`/api/v1/tasks/${encodeURIComponent(taskId)}/children?${buildQuery(params)}`)
}
...
export function searchTasks(
  ...
  return apiFetch(
    `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/task-search?${buildQuery(params)}`,
  )
}
