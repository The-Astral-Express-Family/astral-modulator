// actor 资料域（00012_actor_profile）：PATCH /auth/me 的业务。DTO 复用
// service.go 的 ActorDTO（全仓单一来源）；model.Actor 无 json tag，直接序列化
// 会漏出 PascalCase 字段名，与 openapi/前端的 snake_case 契约漂移。
